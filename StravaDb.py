...
class StravaDb(LeaderBoard):
	...
	def getLeaderboard(self, prevYear, all, elements):
		...
		raw_board = []
		cursor = conn.cursor()
		query_fmt = 'SELECT id, name, CAST(ROUND(dst_{year}) as INT) As "distance" FROM athletes ORDER BY dst_{year} DESC;'
	...
	def getBoard(self, is_all, is_year, is_month, is_previous, elements):
		...
		if is_all:
			...
			if is_previous:
				year = year - 1
			year_next = year + 1
			query_fmt = 'select * from athletes_stats where processed like "{year}%" or processed like "{year_next}-01-01%" order by processed desc;'
